...
const EditDelete = ({ students, onDelete, onUpdate }) => {
  ...
  const handleUpdate = () => {
   ...
   const handleDeleteClick = (studentId) => {
    setDeleteConfirm(studentId); // 👈 open confirmation modal
  };

  const confirmDelete = () => {
    onDelete(deleteConfirm); // 👈 call delete function
    setDeleteConfirm(null);  // close modal
  };

  const cancelDelete = () => {
    setDeleteConfirm(null); // close modal without deleting
  };
          ...
          {students.map((s) => (
            <tr key={s._id} className='min-w-full border-collapse border border-purple-500'>
              ...
              <td className="border border-purple-900 px-4 py-2 text-center">
                ...
                <button onClick={() =>  handleDeleteClick(s._id)}  className="bg-red-400 text-white font-bold px-5 py-2 rounded-[15px] m-5 cursor-pointer hover:bg-red-300">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
