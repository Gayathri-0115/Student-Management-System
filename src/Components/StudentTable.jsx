...
const StudentTable = ({ students, onDelete, onUpdate }) => {
          ...
          {students.map((s) => (
            <tr key={s._id} className='min-w-full border-collapse border border-purple-500'>
              ...
              <td className="border border-purple-900 px-4 py-2 text center">
                <button onClick={() => handleEditClick(s)} className="bg-green-500 text-white px-3 py-1 mx-3 my-3 rounded cursor-pointer hover:bg-green-400">Edit</button>
                <button onClick={() => onDelete(s._id)} className="bg-red-500 text-white px-3 py-1 mx-3 my-3 rounded cursor-pointer hover:bg-red-400">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
